...
static void compute_gravity_error(orientation_filter_t* filter) {
  ...
  if(filter->gravity_estimate_data[3] >= 0){
      ...
      filter->gravity_error_data[0] = -filter->gravity_estimate_data[2] / sqrtf(2 * (1 - filter->gravity_estimate_data[3]));
      filter->gravity_error_data[1] = sqrtf((1 - filter->gravity_estimate_data[3]) / 2);
      filter->gravity_error_data[2] = filter->gravity_estimate_data[1] / sqrtf(2 * (1 - filter->gravity_estimate_data[3]));
      filter->gravity_error_data[3] = 0;
  }
